...
```{r echo=FALSE}
githubDF <- read.csv("/Users/brunodias/Documents/fpcc2/Lab1/githubDF.csv")
```
...
# Reorganizando o dataset inicial para retirar as linguagens que não tiveram atividade no periodo
linguagensUsadas <- 
...
result <- merge(linguagensUsadas, linguagensResult, by = "repository_language", all.x= TRUE) %>%
  na.omit(result)
...
ggplot(trimestresMaisProdutivos, aes(x=repository_language, y=quarter)) + 
  ...
  scale_color_manual(name="Anos", labels=c("2012","2013","2014"), values=c("red","blue","green"))+
  theme_bw() +
...
ggplot(EventosTotaisTri, aes(reorder(quarter, events), y=events/1e+6)) +
  ...
  geom_bar(size=.2, stat="identity", alpha=0.8, width = 0.5, fill=EventosTotaisTri$quarter) + 
  theme_bw()
```
...
# Reorganizando o dataset inicial para retirar as linguagens que não tiveram atividade no periodo
linguagensUsadasTri <- 
  merge(x=EventosPorTrimestre, y=linguagensAtivasTri, by="repository_language", all=TRUE) %>%
  na.omit(linguagensUsadas) %>% select(-n)
...
cor(filter(Qtd_eventosTrimestre, type=="ForkEvent")$events, 
    filter(Qtd_eventosTrimestre, type=="WatchEvent")$events, method = ("pearson"))
```
...
Esse relatório pode ser visualizado no próprio [*Github*](https://github.com/brunodias20/FPCC2/blob/master/Lab1/report_M4.rmd) e no [rPubs](http://rpubs.com/brunodias/L1M4)
